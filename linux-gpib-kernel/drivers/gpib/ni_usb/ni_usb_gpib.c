...
static int ni_usb_nonblocking_receive_bulk_msg(ni_usb_private_t *ni_priv,
	...
	*actual_data_length = 0;
	...
	if ( context->timed_out ) {
		...
		printk("%s: killed urb due to timeout\n", __FUNCTION__);
		retval = -ETIMEDOUT;
	} else if ( ni_priv->bulk_urb->status )	retval = ni_priv->bulk_urb->status;
	del_timer_sync(&ni_priv->bulk_timer);
	*actual_data_length = ni_priv->bulk_urb->actual_length;
	mutex_lock(&ni_priv->bulk_transfer_lock);
	usb_free_urb(ni_priv->bulk_urb);
...
static int ni_usb_parse_register_read_block(const uint8_t *raw_data, unsigned int *results, int num_results) {
	...
	if ( unexpected ) ni_usb_dump_raw_block(raw_data, i);
	return i;
}
...
static int parse_board_ibrd_readback(const uint8_t *raw_data, struct ni_usb_status_block *status,
	...
	while ( raw_data[i] == NIUSB_IBRD_DATA_ID || raw_data[i] == NIUSB_IBRD_EXTENDED_DATA_ID ) {
		...
		for (k = 0; k < data_block_length; k++) {
			if ( j < parsed_data_length ) parsed_data[j++] = raw_data[i++];
			else ++i;
		}
		++num_data_blocks;
	}
...
static int ni_usb_write(gpib_board_t *board, uint8_t *buffer, size_t length, int send_eoi, size_t *bytes_written) {
	...
	*bytes_written = 0;
	...
	out_data[i++] = 0x0;
	out_data[i++] = 0x0;
	if ( send_eoi )	out_data[i++] = 0x8;
	else          	out_data[i++] = 0x0;
	out_data[i++] = 0x0;
	for (j = 0; j < length; j++) out_data[i++] = buffer[j];
	while (i % 4)	// pad with zeros to 4-byte boundary
		out_data[i++] = 0x0;
	i += ni_usb_bulk_termination(&out_data[i]);
...
static int ni_usb_enable_eos(gpib_board_t *board, uint8_t eos_byte, int compare_8_bits) {
	...
	if ( compare_8_bits ) ni_priv->eos_mode |= BIN;
	else                  ni_priv->eos_mode &= ~BIN;
	return 0;
}
...
printk("\n");
	...
	kfree(in_data);
	if ( bsr_bits & BCSR_REN_BIT )  line_status |= BusREN;
	if ( bsr_bits & BCSR_IFC_BIT )  line_status |= BusIFC;
	if ( bsr_bits & BCSR_SRQ_BIT )  line_status |= BusSRQ;
	if ( bsr_bits & BCSR_EOI_BIT )  line_status |= BusEOI;
	if ( bsr_bits & BCSR_NRFD_BIT ) line_status |= BusNRFD;
	if ( bsr_bits & BCSR_NDAC_BIT ) line_status |= BusNDAC;
	if ( bsr_bits & BCSR_DAV_BIT )	line_status |= BusDAV;
	if ( bsr_bits & BCSR_ATN_BIT )	line_status |= BusATN;
	return line_status;
}
...
static void ni_usb_free_private(ni_usb_private_t *ni_priv) {
	if ( ni_priv->interrupt_urb ) usb_free_urb(ni_priv->interrupt_urb);
	kfree(ni_priv);
	return;
}
...
static void ni_usb_cleanup_urbs(ni_usb_private_t *ni_priv) {
	if ( ni_priv && ni_priv->bus_interface ) {
		if ( ni_priv->interrupt_urb ) usb_kill_urb(ni_priv->interrupt_urb);
		if ( ni_priv->bulk_urb ) usb_kill_urb(ni_priv->bulk_urb);
	}
}
...
static int ni_usb_hs_wait_for_ready(ni_usb_private_t *ni_priv) {
	...
	if ( unexpected ) ni_usb_dump_raw_block(buffer, retval);
