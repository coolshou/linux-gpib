...
{
	struct agilent_82357a_priv *a_priv = COMPAT_FROM_TIMER(a_priv, t, bulk_timer);
...
cleanup:
	if (timeout_msecs) {
		if (timer_pending(&a_priv->bulk_timer))
			COMPAT_DEL_TIMER_SYNC(&a_priv->bulk_timer);
	}
...
static int agilent_82357a_attach(struct gpib_board *board, const struct gpib_board_config *config)
{
	...
	product_id = USBID_TO_CPU(usb_dev->descriptor.idProduct);
	switch (product_id) {
